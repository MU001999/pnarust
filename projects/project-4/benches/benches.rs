...
use kvs::{thread_pool::*, KvsClient, Command, Response};
use kvs::{KvStore, KvsEngine, SledKvsEngine, KvsServer};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function_over_inputs("write_queued_kvstore", move |b, &threads| {
        ...
        let server = std::thread::spawn(move || {
            KvsServer::new(logger, addr, engine, thread_pool).unwrap().run(Some(1000)).unwrap();
        });
        ...
        for key in keys {
            b.iter(|| {
                let resp = client.send(Command::Set { key: key.clone(), value: String::from("value") }).unwrap();
    ...
    c.bench_function_over_inputs("read_queued_kvstore", |b, &threads| {
        let temp_dir = TempDir::new().expect("unable to create temporary working directory");
        let path = temp_dir.path().join("db.kvs");
